...
var ALLOWED_EXTS = ['b9', '9', '#9', '11', '#11', 'b13', '13'],
    VALID_EXT_RX = /^[#b]?(?:9|11|13)$/,
    NINTHS =      ['b9', '9', '#9'],
    ELEVENTHS =   ['11', '#11'],
    THIRTEENTHS = ['b13', '13'],
    MODES = Zenino.Core.MODES,
    MODE_NAMES = Zenino.Core.MODE_NAMES;

/**
 * Given an array of intervals in a chord, returns a regexp to find
 * matching modes by UID (@see Zenino.Core.MODES).
 * i.e. [1,3,5,b7,9,#11] returns new RegExp("^123#45[#b]*6b7$");
 **/
function getChordScaleRegex(intervals)
{
  var pattern = '',
      scale = new Array(7),
      interval, i, l;
  // reorder the intervals array so that the 9th is at index 1, 11th at 3 and 13th at 5
  // [1,3,5,b7,9,#11,13] => [1,9,3,#11,5,13,b7]
  for (i = 0, l = intervals.length; i < l; i++) {
    scale[(i*2) % 7] = intervals[i];
  }
  // scale.length = 7
  for (i = 0; i < 7; i++) {
    interval = scale[i];
    if (interval === undefined) {
      ...
      pattern += interval.replace(/9|11|13/, function(match){
        return parseInt(match, 10) - 7; // % 7 is useful only when there are more than 2 octaves
      });
    }
  }
...
var Chords = {
  ...
  fromShortHand: function(root, chord_name)
  {
    ...
    for(i = 0, l = exts.length; i < l; i++) {
      ...
      switch(ext) {
        ...
        case 'b11':
          chord = Zenino.Array.pad(chord, 5, '');
          chord[4] = Zenino.Intervals.diminishedFourth(root);
          break;
        case '11':
        ...
        case '#13':
          chord = Zenino.Array.pad(chord, 7, '');
          chord[6] = Zenino.Intervals.augmentedSixth(root);
          break;
      }
    }
    return chord;
  ...
  isValidExt: function(ext) {
    return VALID_EXT_RX.test(ext);
  },

  /**
   * FIXME: works only for heptatonic modes @see getChordScaleRegex
   **/
  getChordScales: function(chord, sortByBrilliance)
  {
    var root = chord[0],
        scales = [], scale,
        i, l, intervals = ['1'], interval, regex,
        mode_names = sortByBrilliance ? Zenino.Core.MODE_NAMES_BY_BRILLIANCE : Zenino.Core.MODE_NAMES;
        mode_name, mode;
    // computes the intervals from the root
    // intervals becomes like [1,3,5,b7,9,#11,13]
    for (i = 1, l = chord.length; i < l; i++) {
      if (chord[i]) {
        intervals.push(Zenino.Intervals.determine(root, chord[i]));        
      } else {
        intervals.push(undefined);
      }
    }
    // get the regex to test modes interval signature against
    regex = getChordScaleRegex(intervals);
    //console.log('RegExp: '+regex);
    for (i = 0, l = mode_names.length; i < l; i++) {
      var mode_name = mode_names[i],
          mode = MODES[mode_name];
      if (regex.test(mode.uid)) {
        // now get the notes !
        var scale_method = Zenino.Scales[mode_name];
        if (scale_method === undefined) {
          console.log("Mode "+mode_name+" is not implemented");
          continue;
        }
        scale = scale_method(root);
        // console.log(root + ' ' + mode_name + ' : ' + scale);
        scales.push(scale);
      } 
    }
    return scales;
