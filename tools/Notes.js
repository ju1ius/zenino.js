...
var Notes = {
    ...
    throw new FormatError(
      '"'+accidentals+'" is not a valid accidental'
    );
	},
	...
	{
		if(!Notes.isValidNote(note)) throw new NoteFormatError(note);			
  ...
  reduceAccidentals: function(note)
  {
    ...
    for(var i = 1, l = note.length; i < l; i++) {
      ...
      else throw new NoteFormatError(note);
    }
    if(val >= ref)
  ...
  {
    if(!NOTES.hasOwnProperty(name[0])) return false;
